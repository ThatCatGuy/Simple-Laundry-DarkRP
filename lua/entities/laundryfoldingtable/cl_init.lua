...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 240 then
		cam.Start3D2D(pos + ang:Up(), Angle(0, LocalPlayer():EyeAngles().y-90, 90), 0.095)
				draw.SimpleTextOutlined("Folding Table", "ChatFont", 0, -326, white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, black)
				draw.SimpleTextOutlined("Fold clean laundry here.", "ChatFont", 0, -296, white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, black)
		cam.End3D2D()	
	end
end